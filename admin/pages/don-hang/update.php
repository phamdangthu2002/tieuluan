        ...
        <table id="table_js" class="table table-striped" style="width:100%">
            ...
            <tbody>
                ...
                foreach ($chiTietDonHang__Get_By_Id_DH as $item): ?>
                    <tr>
                        ...
                        <td>
                            <?php
                            $sizeInfo = $szsp->SizeSp__Get_By_Id($item->masp);
                            if ($sizeInfo !== null) {
                                echo $sizeInfo->tensize;
                            } else {
                                echo "Không có thông tin kích thước";
                            }
                            ?>
                        </td>
                        <td>
            ...
            <tfoot>
                <tr class="text-center">
                    ...
                    <th colspan="2">
                        <?= number_format($ctdh->ChiTietDonHang__Sum_Tien_DH($item->madon)->sum_tien) ?>
                    </th>
                </tr>
            </tfoot>
        </table>
            ...
            <tbody>
                ...
                foreach ($chiTietTrangThai__Get_By_Id_DH as $item): ?>
                    <tr>
                        ...
                        <td>
                            <?= $tt->TrangThai__Get_By_Id($item->matt)->tentt ?>
                        </td>
                        <td>
                            <?= $item->ngaytao ?>
                        </td>
            ...
            <?php else: ?>
                ...
                <?php foreach ($trangThai__Get_BY_Id_DH as $item): ?>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input checkbox" type="radio" id="matt<?= $item->matt ?>"
                            value="<?= $item->matt ?>" name="matt" required
                            <?= (isset ($cttt->ChiTietTrangThai__Get_Last_By_DH($madon)->matt) ? $cttt->ChiTietTrangThai__Get_Last_By_DH($madon)->matt : "") == $item->matt ? 'checked' : '' ?>>
                        <label class="form-check-label" for="matt<?= $item->matt ?>">
                            <?= $item->tentt ?>
                        </label>
                    </div>
                <?php endforeach; ?>
